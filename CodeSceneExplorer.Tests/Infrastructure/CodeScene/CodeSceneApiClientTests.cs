...
public sealed class CodeSceneApiClientTests
{
    ...
    [Fact]
    public async Task GetKpiTrendHotspotCodeHealthAsync_builds_the_expected_request()
    {
        var requests = new List<HttpRequestMessage>();
        ...
        var result = await sut.GetKpiTrendHotspotCodeHealthAsync(
            42,
        ...
        Assert.Single(requests);
        Assert.Equal(HttpMethod.Get, requests[0].Method);
        Assert.Equal("https://api.codescene.io/v2/projects/42/kpi-trend/code-health/hotspots?start=2025-01-01&end=2025-12-31", requests[0].RequestUri!.ToString());
    }
